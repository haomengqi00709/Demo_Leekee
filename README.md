...
```
**AI 副驾**：OpenAI 兼容接口，DeepSeek / Kimi / OpenAI 任选，设 3 个环境变量即可（不设也能用，走内置正则兜底）：
```bash
# DeepSeek
export LLM_API_KEY=sk-...  LLM_BASE_URL=https://api.deepseek.com     LLM_MODEL=deepseek-chat
# Kimi (Moonshot)
export LLM_API_KEY=sk-...  LLM_BASE_URL=https://api.moonshot.cn/v1  LLM_MODEL=kimi-k2-0905-preview
```
...
3. **环境变量**：
   ...
   - AI 副驾（可选，OpenAI 兼容）：`LLM_API_KEY` + `LLM_BASE_URL` + `LLM_MODEL`
     - DeepSeek：`LLM_BASE_URL=https://api.deepseek.com` · `LLM_MODEL=deepseek-chat`
     - Kimi：`LLM_BASE_URL=https://api.moonshot.cn/v1` · `LLM_MODEL=kimi-k2-0905-preview`
