...
```
**AI 副驾**：设 `export ANTHROPIC_API_KEY=sk-...` 走真 Claude（默认 `claude-opus-4-8`，可用 `COPILOT_MODEL=claude-haiku-4-5` 换更省的）；不设也能用（内置正则兜底）。
...
3. **环境变量**：
   ...
   - `ANTHROPIC_API_KEY=sk-...`（可选，开 AI 副驾）
   - `COPILOT_MODEL=claude-haiku-4-5`（可选，默认 opus）
